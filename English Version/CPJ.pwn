...
public OnFilterScriptInit()
{
    ...
    {
        // Error
        print("Failed to open a connection to \"CP-Data.db\".");
    ...
    else
    {
        ...
        db_free_result(db_query(db_handle, "CREATE TABLE IF NOT EXISTS `pData`(`ID` INTEGER PRIMARY KEY AUTOINCREMENT,`xPos` FLOAT NOT NULL, `yPos` FLOAT NOT NULL, `zPos` FLOAT NOT NULL, `aPos` FLOAT NOT NULL, `pInt` INTEGER NOT NULL, `pWorld` INTEGER NOT NULL, `PlayerName` VARCHAR(24) NOT NULL)"));
        print("Successfully created a connection to \"CP-Data.db\".");
    }
...
public DelayPunishment(playerid)
{
    if(pJailed[playerid] == true)
    {
        ...
        SendClientMessage(playerid, COLOR_GREEN, "You've been put back in jail because you weren't finished with the punishment!");
        SendClientMessage(playerid, COLOR_GREEN, "Run through all Checkpoints to get out here. Behave next time!");
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
    ...
    if(CheckPointCounter[playerid] == MaxCheckPoints[playerid] && pJailed[playerid] == true)
    {
        ...
        pJailed[playerid] = false;
        CheckPointCounter[playerid] = 0;
        format(str, sizeof(str),"{%06x}%s {80FF00}is finished with the Checkpoint Jail Punishment!", GetPlayerColor(playerid) >>> 8, GetName(playerid));
        SendClientMessageToAll(COLOR_GREEN, str);
        SendClientMessage(playerid, COLOR_RED, "You're done! Learn from your mistakes and behave in future!");
...
CMD:cpjail(playerid, params[])
{
    ...
    SendClientMessage(playerid, COLOR_RED, "Use: /cpjail <Name/ID> <Reason> <Checkpoints>");
    ...
    {
        return SendClientMessage(playerid, COLOR_RED, "The player is already inside the Checkpoint Jail.");
    }

    if(GetPlayerState(target) != PLAYER_STATE_ONFOOT)
    {
        return SendClientMessage(playerid, COLOR_RED, "The player is in a vehicle or not spawned yet!");
    }
    ...
    format(str, sizeof(str), "{%06x}%s {80FF00}has punished {%06x}%s {80FF00}with CP Jail(%d). Reason: (%s)", GetPlayerColor(playerid) >>> 8, GetName(playerid), GetPlayerColor(target) >>> 8, GetName(target), cps,reason);
    SendClientMessageToAll(COLOR_GREEN , str);
    SendClientMessage(target, COLOR_GREEN, "Run through all Checkpoints to get out here. Behave next time!");
...
CMD:cpunjail(playerid, params[])
{
    ...
    {
        return SendClientMessage(playerid, COLOR_RED, "The player is not inside the Checkpoint Jail");
    }
    ...
    format(str, sizeof(str),"{%06x}%s {80FF00}has unjailed {%06x}%s {80FF00}from the CP Jail.", GetPlayerColor(playerid) >>> 8, GetName(playerid), GetPlayerColor(target) >>> 8, GetName(target));
    SendClientMessageToAll(COLOR_GREEN, str);
...
CMD:prisonlist(playerid)
{
    ...
    if(pCount > 0)
    {
        ShowPlayerDialog(playerid, D_PRISONLIST, DIALOG_STYLE_MSGBOX, "Punished Players", cQuery, "Close", "");
    }
    else
    {
        SendClientMessage(playerid, COLOR_RED, "There are no players inside the Checkpoint Jail!");
    }
    return 1;
}
...
CreateCPTextDraws(playerid)
{
    ...
    CheckpointTD[playerid][3] = CreatePlayerTextDraw(playerid,38.000000, 265.000000, "~r~~h~C~w~heckpoints~n~      ~r~~h~l~w~eft");
...
#if MIN_CPS > MAX_CPS
    #error Maximum Checkpoints can't be lower than minimum checkpoints! Increase the value of #define MAX_CPS !
#endif


#if PUNISH_DELAY < 3
    #error Minimum Punishment Delay is 3 seconds! Increase the value on #define PUNISH_DELAY
#endif

#if SAVE_WEAPONS < 0 || SAVE_WEAPONS > 1
    #error Unknown setting, use #define SAVE_WEAPONS 1 to enable saving & loading weapons and #define SAVE_WEAPONS 0 to disable it!
#endif

#if DIFFERENT_WORLD < 0 || DIFFERENT_WORLD > 1
    #error Unknown setting, use #define DIFFERENT_WORLD 1 to enable spawning in different virtual worlds and #define DIFFERENT_WORLD 0 to let all players spawn in the same world!
#endif

#if PRO_PUNISHMENT < 0 || PRO_PUNISHMENT > 1
    #error Unknown setting, use #define PRO_PUNISHMENT 1 or 0 to enable / disable the setting!
#endif

#if SHOW_RULES < 0 || SHOW_RULES > 1
    #error Unknown setting, use #define SHOW_RULES 1 or 0 to enable / disable the setting!
#endif
